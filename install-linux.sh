...
# ---------------------------------------------------------------------------
# 13) WezTerm plugins (cloned manually, not via wezterm.plugin.require).
...
#     No-op if dotter hasn't created ~/.config/wezterm yet (typical on WSL,
#     where wezterm runs on the Windows host, not inside WSL).
# ---------------------------------------------------------------------------
...
if [[ -d "$WEZTERM_DIR" ]]; then
  ...
  if [[ -f "$SM_FILE" ]] && grep -q '/.config/wezterm/wezterm-session-manager/wezterm_state_' "$SM_FILE"; then
    ...
    # GNU sed in-place edit; no backup file.
    sed -i \
