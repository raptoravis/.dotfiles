...
description: Refresh context by saving a handoff, committing and pushing the work, clearing the session, and resuming. Use when the conversation has grown large and you want a clean context window while preserving working state on disk and in git. Triggers on phrases like "refresh context", "reset with handoff", "context too big — restart", or explicit /handoff-refresh.
---

# handoff-refresh — four-step context refresh

Goal: free up context-window tokens **without losing working state** by chaining
`handoff:create` → `commit` + `git push` → user-side `/clear` → `handoff:resume`.
...
1. **Before `/clear`** — invoke `handoff:create` so working state is captured
   to disk, then commit and push so the snapshot is durable in git.
...
wrote), capture that path for the message in Step 3.

### Step 2 — Commit and push

Invoke the **`commit`** skill via the Skill tool to stage changes (including
the new/updated `HANDOFF.md`) and create a well-formatted commit. After the
commit lands, run `git push` so the snapshot is safe on the remote before the
session is wiped.

Guardrails:
- If there is nothing to commit (clean working tree apart from the handoff,
  and the handoff is already committed), skip the commit but still push any
  unpushed commits.
- If the current branch has no upstream, push with `git push -u origin
  <branch>`.
- If `git push` fails (auth, network, protected branch), surface the error
  and **stop** — do not proceed to Step 3. The user should not `/clear`
  until their work is durable.
- Never use `--no-verify`, `--force`, or amend prior commits unless the user
  explicitly asks for it.

### Step 3 — Hand off to the user

Stop tool calls. Print exactly this block to the user (substitute the actual
handoff path and commit SHA):

```
Handoff written to <path/to/HANDOFF.md> and pushed as <short-sha> on <branch>.
...
`handoff:resume` looks), skip Steps 1–2 entirely and instead invoke
...
Otherwise treat it as "pre-`/clear` state" and run Steps 1–2.
...
- Don't draft commit messages by hand either — delegate to the `commit` skill
  so the repo's conventional-commit style is preserved.
- If the project already had a stale `HANDOFF.md` from days ago, mention it
  in Step 3 so the user knows the file is being overwritten.
- This skill assumes the `handoff` plugin is enabled in
  ...
  plugin.
- If the working directory is not a git repository, skip Step 2 and note in
  Step 3 that no commit/push was performed.
